...
本目录当前 74 节，包含 14 个执行器与观测主题组；客户端协议/plan cache、表达式与 FMGR、执行期分区路由课程已经补齐。
...
12. Frontend/backend protocol、Portal 与 plan cache。
13. Type / Function / Expression / FMGR / JIT。
14. Partition routing executor side。
...
以下课程保留在 README 中作为建议阅读路径；编号沿用追加编号，未重命名已有 01-56。
...
1. [frontend/backend protocol message loop](57-frontend-backend-protocol-message-loop.md)：post-auth backend 如何在 frontend/backend protocol message loop 中分派 simple query 和 extended query？
2. [Simple Query 如何进入 transaction command loop](58-simple-query-transaction-command.md)：simple query 如何进入 transaction command loop、parse/analyze/rewrite/plan/execute，并处理隐式事务边界？
3. [extended query Parse/Bind/Describe/Execute/Sync 边界](59-extended-query-parse-bind-execute.md)：Parse/Bind/Describe/Execute/Sync 如何把 prepared statement、portal 和 executor 分成多个协议阶段？
4. [prepared statement 与 plan cache](60-prepared-statement-plan-cache.md)：prepared statement 如何保存 raw/analyzed query、generic/custom plan 和参数类型？
5. [cached plan invalidation](61-cached-plan-invalidation.md)：catalog invalidation、search_path、GUC 和 relcache 变化如何让 cached plan 失效或重建？
6. [Portal lifecycle / suspended execution](62-portal-lifecycle-suspended-execution.md)：Portal 如何支持 cursor、FETCH、暂停执行、holdable cursor 和 ERROR cleanup？
7. [DestReceiver / output protocol](63-destreceiver-output-protocol.md)：DestReceiver 如何把 executor tuple 输出到客户端、SPI、tuplestore、COPY 或 EXPLAIN？
8. [plan cache / executor boundary](64-plan-cache-executor-boundary.md)：plan cache 输出的 `PlannedStmt` 如何成为 executor 可执行契约，哪些状态仍必须运行期补齐？

第 13 项 `Type / Function / Expression / FMGR / JIT` 建议插在第 2 项 PlanState / TupleTableSlot / ExprContext 之后、第 4 项常见执行节点之前阅读：

1. [type input/output、varlena、TOAST 与 detoast](65-type-input-output-varlena.md)：type input/output、typmod、varlena、TOAST/detoast 如何影响 executor 中的 Datum 生命周期？
2. [fmgr function call boundary](66-fmgr-function-call-boundary.md)：fmgr 如何用 `FmgrInfo`、`FunctionCallInfo`、strict、set-returning function 和 memory context 调用 C 函数？
3. [operator/function lookup execution](67-operator-function-lookup-execution.md)：operator、support function 和 function cache 如何把 catalog metadata 转成可调用执行状态？
4. [ExecEval expression interpreter](68-exec-eval-interpreter.md)：表达式解释器如何把 `ExprState` steps 执行成 Datum / null，短生命周期内存在哪里 reset？
5. [expression deform / detoast cost](69-expression-deform-detoast.md)：slot deform、attribute cache、detoast 和 pass-by-value/reference 如何放大表达式成本？
6. [Collation / Sort Support / Comparison Function](70-collation-sort-support.md)：collation、sort support 和 comparison function 如何影响 sort、merge join、btree scan 和 hash equality？
7. [JIT expression / deform boundary](71-jit-expression-deform-boundary.md)：LLVM JIT 何时接管 expression / deform，为什么 JIT startup cost 和 plan cache 状态必须一起判断？
8. [expression ERROR 与 memory cleanup](72-expression-error-memory-cleanup.md)：函数 ERROR、SRF 中断和表达式短路如何保证 per-tuple memory、FunctionCallInfo 和 ResourceOwner 收尾？

第 14 项 `Partition routing executor side` 建议插在第 4 项 `ModifyTable` 之后阅读：

1. [partition routing 与 ModifyTable](73-partition-routing-modifytable.md)：INSERT/UPDATE 如何在 executor 中选择目标分区，partition constraint、tuple conversion 和 trigger 顺序如何协作？
2. [partition tuple conversion 与 pruning boundary](74-partition-tuple-conversion-pruning-boundary.md)：执行期 partition pruning、tuple routing 和 row movement 如何与 planner 分区路径、ModifyTable 和 ResultRelInfo 对接？
