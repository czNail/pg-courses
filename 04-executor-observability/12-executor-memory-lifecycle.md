# PostgreSQL QueryDesc、EState 与 executor context 生命周期

## 课程定位

前置知识：熟悉 MemoryContext tree、QueryDesc、ExecutorStart
/ Run / Finish / End 调用顺序。

本节唯一主问题：

```text
一次查询执行中，哪些状态应该挂在 EState 的 executor context 下，并由 ExecutorEnd 整体释放？
```

核心矛盾：执行器需要让大量节点、表达式、slot、ResultRelInfo
和辅助状态共享同一个查询生命周期；但外部资源、buffer pin、relation
refcount 和 callback 又不能只靠删除内存上下文。
...
本节只回答一个问题：一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？
...
```text
CreateQueryDesc 创建执行描述符
ExecutorStart 调用 CreateExecutorState
PlanState、ExprContext 和 slot table 挂在 es_query_cxt 下
ExecutorEnd 先关闭节点和资源，再 FreeExecutorState
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/include/executor/execdesc.h | `QueryDesc` 保存 PlannedStmt、snapshot、dest、estate 和 planstate。 |
| 2 | src/backend/tcop/pquery.c | `CreateQueryDesc()`、`FreeQueryDesc()`。 |
| 3 | src/backend/executor/execMain.c | `ExecutorStart()`、`ExecutorRun()`、`ExecutorFinish()`、`ExecutorEnd()`。 |
| 4 | src/backend/executor/execUtils.c | `CreateExecutorState()`、`FreeExecutorState()`。 |
| 5 | src/backend/executor/execProcnode.c | `ExecInitNode()`、`ExecEndNode()` 递归管理 PlanState tree。 |
| 6 | src/backend/executor/execTuples.c | `ExecResetTupleTable()` 清理 slot 资源。 |
| 7 | src/backend/executor/README | per-query context、per-tuple context 和 cleanup 顺序说明。 |
...
| 状态 | 本节语义 |
| --- | --- |
| QueryDesc | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| EState | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| es_query_cxt | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| PlanState tree | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| es_tupleTable | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| es_exprcontexts | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`QueryDesc`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `QueryDesc` 时要同时记录所在 memory context 或上层
...
诊断 `EState` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `es_query_cxt` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `PlanState tree` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `es_tupleTable` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `es_exprcontexts` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
```text
CreateQueryDesc  -> 改变或消费本节主状态
ExecutorStart  -> 改变或消费本节主状态
ExecInitNode  -> 改变或消费本节主状态
ExecutorRun  -> 改变或消费本节主状态
ExecutorFinish  -> 改变或消费本节主状态
ExecutorEnd  -> 改变或消费本节主状态
FreeExecutorState  -> 改变或消费本节主状态
FreeQueryDesc  -> 改变或消费本节主状态
```

第 1 步看
`CreateQueryDesc`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `CreateQueryDesc` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `CreateQueryDesc` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 2 步看
`ExecutorStart`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecutorStart` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecutorStart` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 3 步看
`ExecInitNode`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecInitNode` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecInitNode` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 4 步看
`ExecutorRun`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecutorRun` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecutorRun` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 5 步看
`ExecutorFinish`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecutorFinish` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecutorFinish` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
第 7 步看
`FreeExecutorState`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `FreeExecutorState` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `FreeExecutorState` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 8 步看
`FreeQueryDesc`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `FreeQueryDesc` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `FreeQueryDesc` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
pg_backend_memory_contexts 观察 ExecutorState。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

断点 CreateExecutorState。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

断点 ExecResetTupleTable。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

长 cursor 观察 context 生命周期。
...
误区：看到 `QueryDesc` 就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和
cleanup 边界。

误区：看到 `EState` 就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和
cleanup 边界。

误区：看到 `es_query_cxt`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `PlanState tree`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
BEGIN;
DECLARE c CURSOR FOR SELECT * FROM generate_series(1,100000);
SELECT name,total_bytes FROM pg_backend_memory_contexts WHERE name = 'ExecutorState';
CLOSE c; COMMIT;
```
...
练习 1：打开
`/home/highgo/postgres/src/include/executor/execdesc.h`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/include/executor/execdesc.h` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 2：打开
`/home/highgo/postgres/src/backend/tcop/pquery.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/tcop/pquery.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
练习 4：打开
`/home/highgo/postgres/src/backend/executor/execUtils.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execUtils.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 5：打开
`/home/highgo/postgres/src/backend/executor/execProcnode.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execProcnode.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 6：打开
`/home/highgo/postgres/src/backend/executor/execTuples.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execTuples.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 7：打开
`/home/highgo/postgres/src/backend/executor/README`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/README` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

### 14.1 从现象回到源码的排查剧本

压缩用户现象：围绕 `一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `一次查询执行中，哪些状态应该挂在 EState 的 executor
context 下，并由 ExecutorEnd 整体释放？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `一次查询执行中，哪些状态应该挂在 EState 的
executor context 下，并由 ExecutorEnd
整体释放？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `executor 生命周期边界` 放回真实执行现场。主角是
QueryDesc、EState、ExecutorState context，主线是
ExecutorStart 到 ExecutorEnd。
...
套回本节时，先写下 QueryDesc、EState、ExecutorState context
中哪个对象被创建、读取、失效或释放，再解释它如何影响 ExecutorStart 到
ExecutorEnd。
...
在 `executor 生命周期边界`
中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
