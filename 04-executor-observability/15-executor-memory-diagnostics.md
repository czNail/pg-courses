# PostgreSQL Executor 内存诊断与峰值解释

## 课程定位

前置知识：熟悉 MemoryContext tree、EXPLAIN
ANALYZE、work_mem、Sort/Hash/Agg 节点和
pg_backend_memory_contexts。

本节唯一主问题：

```text
面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨 tuple retention、spill 前膨胀和真正 leak？
```

核心矛盾：用户希望一个数字说明这条 SQL 用了多少内存；但执行器内存分布在
per-query、per-tuple、节点私有 context、instrumentation
和临时文件之间，且不同指标采样边界不同。
...
本节只回答一个问题：面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正 leak？
...
```text
EXPLAIN 给节点级执行事实
pg_backend_memory_contexts 给当前 context tree
pg_log_backend_memory_contexts 给日志快照
源码 counters 说明指标来源
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/backend/utils/adt/mcxtfuncs.c | `pg_get_backend_memory_contexts()`、`pg_log_backend_memory_contexts()`。 |
| 2 | src/backend/catalog/system_views.sql | `pg_backend_memory_contexts` 视图定义。 |
| 3 | src/backend/utils/mmgr/mcxt.c | `MemoryContextStatsDetail()`、`MemoryContextMemAllocated()`。 |
| 4 | src/backend/commands/explain.c | `show_memory_counters()` 和节点 Explain memory 输出。 |
| 5 | src/backend/executor/nodeAgg.c | `hash_agg_update_metrics()`、`hash_agg_check_limits()`。 |
| 6 | src/backend/executor/nodeHash.c | `ExecHashAccumInstrumentation()` 汇总 Hash 指标。 |
| 7 | src/backend/executor/execMain.c | `ExecutorStart()`、`ExecutorEnd()` 决定 ExecutorState 窗口。 |
...
| 状态 | 本节语义 |
| --- | --- |
| MemoryContextCounters | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| pg_backend_memory_contexts row | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| Explain memory counters | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| HashAgg metrics | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| Hash instrumentation | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ExecutorState context | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`MemoryContextCounters`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `MemoryContextCounters` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。

`pg_backend_memory_contexts row`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `pg_backend_memory_contexts row` 时要同时记录所在
memory context 或上层 owner。executor 中大量 bug
和误判都来自把裸指针当成长期语义。

`Explain memory counters`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `Explain memory counters` 时要同时记录所在 memory
context 或上层 owner。executor 中大量 bug
和误判都来自把裸指针当成长期语义。
...
诊断 `HashAgg metrics` 时要同时记录所在 memory context 或上层
...
诊断 `Hash instrumentation` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ExecutorState context` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
执行 SQL  -> 改变或消费本节主状态
节点运行  -> 改变或消费本节主状态
EXPLAIN ANALYZE  -> 改变或消费本节主状态
pg_backend_memory_contexts  -> 改变或消费本节主状态
pg_log_backend_memory_contexts  -> 改变或消费本节主状态
MemoryContextMemAllocated  -> 改变或消费本节主状态
ExecutorEnd  -> 改变或消费本节主状态
诊断闭环  -> 改变或消费本节主状态
```

第 1 步看 `执行
SQL`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `执行 SQL` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
...
第 2 步看
`节点运行`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `节点运行` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `节点运行` 是 hot path，调试时用条件断点和计数，不要盲目逐步执行。executor
的调用栈很深，关键是状态边界。

第 3 步看 `EXPLAIN
ANALYZE`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `EXPLAIN ANALYZE` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `EXPLAIN ANALYZE` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 4 步看
`pg_backend_memory_contexts`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `pg_backend_memory_contexts` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `pg_backend_memory_contexts` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 5 步看
`pg_log_backend_memory_contexts`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `pg_log_backend_memory_contexts`
前要问调用者能依赖什么：一个有效 slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `pg_log_backend_memory_contexts` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 6 步看
`MemoryContextMemAllocated`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `MemoryContextMemAllocated` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `MemoryContextMemAllocated` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 7 步看
`ExecutorEnd`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecutorEnd` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecutorEnd` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 8 步看
`诊断闭环`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `诊断闭环` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `诊断闭环` 是 hot path，调试时用条件断点和计数，不要盲目逐步执行。executor
的调用栈很深，关键是状态边界。
...
误区：看到 `MemoryContextCounters`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `pg_backend_memory_contexts row`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `Explain memory counters`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `HashAgg metrics`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
SELECT name,type,level,total_bytes,used_bytes FROM pg_backend_memory_contexts ORDER BY total_bytes DESC LIMIT 20;
EXPLAIN (ANALYZE, VERBOSE, BUFFERS, MEMORY) SELECT g % 100000, count(*) FROM generate_series(1,1000000) g GROUP BY 1;
```
...
```text
break 节点运行
break pg_backend_memory_contexts
run
```
...
练习 1：打开
`/home/highgo/postgres/src/backend/utils/adt/mcxtfuncs.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/utils/adt/mcxtfuncs.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 2：打开
`/home/highgo/postgres/src/backend/catalog/system_views.sql`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/catalog/system_views.sql` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 3：打开
`/home/highgo/postgres/src/backend/utils/mmgr/mcxt.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/utils/mmgr/mcxt.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 4：打开
`/home/highgo/postgres/src/backend/commands/explain.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/commands/explain.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 5：打开
`/home/highgo/postgres/src/backend/executor/nodeAgg.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/nodeAgg.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 6：打开
`/home/highgo/postgres/src/backend/executor/nodeHash.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/nodeHash.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
压缩用户现象：围绕 `面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `面对 ExecutorState 或节点内存峰值，如何区分正常峰值、跨
tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `面对 ExecutorState
或节点内存峰值，如何区分正常峰值、跨 tuple retention、spill 前膨胀和真正
leak？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `内存诊断口径边界` 放回真实执行现场。主角是 EXPLAIN、context
view、日志快照、节点指标，主线是 可见数字到源码状态。
...
套回本节时，先写下 EXPLAIN、context view、日志快照、节点指标
中哪个对象被创建、读取、失效或释放，再解释它如何影响 可见数字到源码状态。
...
在 `内存诊断口径边界` 中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
