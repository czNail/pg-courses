# PostgreSQL ExprContext 与 per-tuple 表达式状态边界

## 课程定位

前置知识：熟悉 PlanState、TupleTableSlot、targetlist / qual
求值，以及 MemoryContext tree 的基本语义。

本节唯一主问题：

```text
表达式求值为什么必须通过 ExprContext 携带当前 tuple、参数和短生命周期内存，而不能只把 Datum 参数逐层传进去？
```

核心矛盾：表达式执行要足够快、足够通用，能处理 scan/inner/outer
tuple、聚合、窗口、参数和函数调用；同时每一行产生的临时内存必须在正确边界批量释放。
...
本节只回答一个问题：表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum 参数逐层传进去？
...
```text
PlanState 创建 ExprContext
ExprContext 指向 scan、inner、outer slot
表达式在 ecxt_per_tuple_memory 中产生短命结果
节点在 tuple 周期调用 ResetExprContext
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/include/nodes/execnodes.h | `ExprContext`、`ExprState`、`ProjectionInfo` 定义表达式现场。 |
| 2 | src/include/executor/executor.h | `ResetExprContext`、`GetPerTupleExprContext`、`ResetPerTupleExprContext`。 |
| 3 | src/backend/executor/execUtils.c | `CreateExprContext()`、`ExecAssignExprContext()`、`FreeExprContext()`。 |
| 4 | src/backend/executor/execExpr.c | `ExecInitExpr()`、`ExecBuildProjectionInfo()` 编译表达式。 |
| 5 | src/include/executor/execScan.h | `ExecScanExtended()` 设置 scan tuple、qual 和 projection。 |
| 6 | src/backend/executor/nodeHashjoin.c | `ExecHashJoinImpl()` 展示 outer/inner slot 切换。 |
| 7 | src/backend/executor/nodeAgg.c | `tmpcontext`、`aggcontext` 与 per-tuple context 的层次差异。 |
...
| 状态 | 本节语义 |
| --- | --- |
| ecxt_scantuple | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ecxt_innertuple | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ecxt_outertuple | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ecxt_per_tuple_memory | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ecxt_per_query_memory | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ecxt_callbacks | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`ecxt_scantuple`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `ecxt_scantuple` 时要同时记录所在 memory context 或上层
...
诊断 `ecxt_innertuple` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ecxt_outertuple` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ecxt_per_tuple_memory` 时要同时记录所在 memory context
...
诊断 `ecxt_per_query_memory` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ecxt_callbacks` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
CreateExprContext  -> 改变或消费本节主状态
ExecInitExpr  -> 改变或消费本节主状态
ExecScanExtended  -> 改变或消费本节主状态
ExecQual  -> 改变或消费本节主状态
ExecProject  -> 改变或消费本节主状态
ResetExprContext  -> 改变或消费本节主状态
...
第 2 步看
`CreateExprContext`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `CreateExprContext` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `CreateExprContext` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 3 步看
`ExecInitExpr`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecInitExpr` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecInitExpr` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
第 5 步看
`ExecQual`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecQual` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。
...
第 7 步看
`ResetExprContext`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ResetExprContext` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `ResetExprContext` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
pg_backend_memory_contexts 观察 ExprContext。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

gdb 打印 ps_ExprContext slot。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

断点 ExecScanExtended 和 ExecProject。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

profile 中观察 ExecInterpExpr 热点。
...
误区：看到 `ecxt_scantuple`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `ecxt_innertuple`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `ecxt_outertuple`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `ecxt_per_tuple_memory`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
CREATE TABLE t_expr AS SELECT g, md5(g::text) v FROM generate_series(1,10000) g;
EXPLAIN (ANALYZE, VERBOSE) SELECT upper(v) FROM t_expr WHERE g % 2 = 0;
```
...
压缩用户现象：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `表达式求值为什么必须通过 ExprContext 携带当前
tuple、参数和短生命周期内存，而不能只把 Datum
参数逐层传进去？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `表达式求值现场` 放回真实执行现场。主角是
ExprContext、ExprState、per-tuple memory，主线是 当前
tuple 到表达式结果。
...
套回本节时，先写下 ExprContext、ExprState、per-tuple memory
中哪个对象被创建、读取、失效或释放，再解释它如何影响 当前 tuple 到表达式结果。
...
在 `表达式求值现场` 中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
