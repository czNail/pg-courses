# PostgreSQL TupleDesc、投影与 junk attribute 的输出形状边界

## 课程定位

前置知识：熟悉 TargetEntry、TupleTableSlot、PlanState
targetlist，以及 UPDATE / DELETE 需要 row identity
的基本语义。

本节唯一主问题：

```text
执行器如何在不破坏内部行身份的前提下，把节点内部 tuple 形状转换成上层需要的输出形状？
```

核心矛盾：节点内部需要携带 ctid、tableoid、sort key、row identity
等 executor-only 字段；用户可见结果、RETURNING、DestReceiver
和写入路径又只能接收干净的 tuple 形状。
...
本节只回答一个问题：执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？
...
```text
plan targetlist 描述要算什么列
TupleDesc 描述 slot 承诺的行形状
ProjectionInfo 把表达式写入结果 slot
JunkFilter 在顶层或 DML 边界移除 executor-only 字段
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/include/executor/tuptable.h | `TupleTableSlot`、`TupleTableSlotOps` 和 descriptor / values / isnull 边界。 |
| 2 | src/include/access/tupdesc.h | `TupleDescData`、`TupleDescAttr()`、`TupleDescFinalize()` 描述行形状。 |
| 3 | src/include/nodes/execnodes.h | `ProjectionInfo`、`JunkFilter`、`ExprState` 的运行时状态。 |
| 4 | src/backend/executor/execExpr.c | `ExecBuildProjectionInfo()` 编译 targetlist。 |
| 5 | src/include/executor/executor.h | `ExecProject()` 形成 virtual result slot。 |
| 6 | src/backend/executor/execJunk.c | `ExecInitJunkFilter()`、`ExecFilterJunk()` 删除 junk 列。 |
| 7 | src/backend/executor/execMain.c | `ExecutePlan()` 在顶层输出前应用 junk filter。 |
| 8 | src/backend/executor/execScan.c | `ExecAssignScanProjectionInfo()` 决定直通还是投影。 |
...
| 状态 | 本节语义 |
| --- | --- |
| TupleDesc | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| TupleTableSlot | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ProjectionInfo | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| TargetEntry.resjunk | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| JunkFilter.cleanMap | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ResultTupleSlot | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`TupleDesc`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `TupleDesc` 时要同时记录所在 memory context 或上层
...
诊断 `TupleTableSlot` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ProjectionInfo` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `TargetEntry.resjunk` 时要同时记录所在 memory context
...
诊断 `JunkFilter.cleanMap` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `ResultTupleSlot` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
ExecAssignScanProjectionInfo  -> 改变或消费本节主状态
ExecBuildProjectionInfo  -> 改变或消费本节主状态
ExecProcNode  -> 改变或消费本节主状态
ExecProject  -> 改变或消费本节主状态
ExecutePlan  -> 改变或消费本节主状态
ExecFilterJunk  -> 改变或消费本节主状态
DestReceiver  -> 改变或消费本节主状态
```
...
第 2 步看
`ExecAssignScanProjectionInfo`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecAssignScanProjectionInfo` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `ExecAssignScanProjectionInfo` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 3 步看
`ExecBuildProjectionInfo`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecBuildProjectionInfo` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `ExecBuildProjectionInfo` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 4 步看
`ExecProcNode`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecProcNode` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecProcNode` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 5 步看
`ExecProject`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecProject` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecProject` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 6 步看
`ExecutePlan`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecutePlan` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecutePlan` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 7 步看
`ExecFilterJunk`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecFilterJunk` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecFilterJunk` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 8 步看
`DestReceiver`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `DestReceiver` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `DestReceiver` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
EXPLAIN (VERBOSE) 观察 targetlist。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

gdb 断点 ExecFilterJunk。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

比较 ss_ScanTupleSlot 与 ps_ResultTupleSlot。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

宽表投影时观察 slot_getattr 和 ExecProject。
...
误区：看到 `TupleDesc` 就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和
cleanup 边界。

误区：看到 `TupleTableSlot`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `ProjectionInfo`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `TargetEntry.resjunk`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
CREATE TABLE t_junk(id int primary key, v text);
EXPLAIN (ANALYZE, VERBOSE) UPDATE t_junk SET v = v || 'x' WHERE id = 1 RETURNING v;
```
...
```text
break ExecAssignScanProjectionInfo
break ExecProcNode
run
```
...
练习 1：打开
`/home/highgo/postgres/src/include/executor/tuptable.h`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/include/executor/tuptable.h` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 2：打开
`/home/highgo/postgres/src/include/access/tupdesc.h`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/include/access/tupdesc.h` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
练习 5：打开
`/home/highgo/postgres/src/include/executor/executor.h`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/include/executor/executor.h` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 6：打开
`/home/highgo/postgres/src/backend/executor/execJunk.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execJunk.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
练习 8：打开
`/home/highgo/postgres/src/backend/executor/execScan.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execScan.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

### 14.1 从现象回到源码的排查剧本

压缩用户现象：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部 tuple
形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `执行器如何在不破坏内部行身份的前提下，把节点内部
tuple 形状转换成上层需要的输出形状？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `输出形状边界` 放回真实执行现场。主角是
TupleDesc、ProjectionInfo、JunkFilter，主线是 dirty slot
到 clean slot。
...
套回本节时，先写下 TupleDesc、ProjectionInfo、JunkFilter
中哪个对象被创建、读取、失效或释放，再解释它如何影响 dirty slot 到 clean
slot。
...
在 `输出形状边界` 中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
