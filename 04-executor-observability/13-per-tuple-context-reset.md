# PostgreSQL per-tuple context reset 与表达式泄漏边界

## 课程定位

前置知识：熟悉 ExprContext、slot virtual
tuple、MemoryContext reset，以及表达式求值可能分配 varlena /
array / json / function state。

本节唯一主问题：

```text
为什么执行器要在每个 tuple 周期 reset per-tuple context，哪些结果必须复制或 materialize 才能跨周期保存？
```

核心矛盾：表达式、函数、过滤器和投影每行都会产生短命对象；逐个 pfree 成本高且 ERROR
不安全，但过度批量保留又会让内存随行数增长。
...
本节只回答一个问题：为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize 才能跨周期保存？
...
```text
表达式短命结果进入 per-tuple memory
scan、join、project 和 ExecutePlan 在 tuple 边界 reset
跨边界保存必须复制或 materialize
下一轮 reset 让未保存指针失效
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/include/executor/executor.h | `ResetExprContext`、`ResetPerTupleExprContext`、`GetPerTupleMemoryContext`。 |
| 2 | src/include/nodes/execnodes.h | `ExprContext.ecxt_per_tuple_memory` 与 `ExprState.resultslot`。 |
| 3 | src/backend/executor/execMain.c | `ExecutePlan()` 每轮顶层 tuple reset。 |
| 4 | src/include/executor/execScan.h | `ExecScanExtended()` 每轮 scan tuple reset。 |
| 5 | src/backend/executor/nodeHashjoin.c | Hash Join 匹配、补 null、批次推进中的 reset。 |
| 6 | src/backend/executor/nodeSubplan.c | hashed SubPlan 清理投影 slot 与 inner context。 |
| 7 | src/backend/executor/execTuples.c | `ExecMaterializeSlot()`、`ExecCopySlot()` 处理跨边界保存。 |
...
| 状态 | 本节语义 |
| --- | --- |
| per-output-tuple ExprContext | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| PlanState ps_ExprContext | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| virtual tuple result | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| materialized slot | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| ExprContext callback | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| tuple retention structure | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`per-output-tuple ExprContext`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `per-output-tuple ExprContext` 时要同时记录所在 memory
...
诊断 `PlanState ps_ExprContext` 时要同时记录所在 memory
context 或上层 owner。executor 中大量 bug
和误判都来自把裸指针当成长期语义。

`virtual tuple result`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `virtual tuple result` 时要同时记录所在 memory context
...
诊断 `materialized slot` 时要同时记录所在 memory context 或上层
...
诊断 `ExprContext callback` 时要同时记录所在 memory context
或上层 owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。

`tuple retention structure`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `tuple retention structure` 时要同时记录所在 memory
context 或上层 owner。executor 中大量 bug
和误判都来自把裸指针当成长期语义。
...
ExecMaterializeSlot  -> 改变或消费本节主状态
tuplestore_puttupleslot  -> 改变或消费本节主状态
下一轮 reset  -> 改变或消费本节主状态
```
...
第 6 步看
`ExecMaterializeSlot`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecMaterializeSlot` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `ExecMaterializeSlot` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 7 步看
`tuplestore_puttupleslot`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `tuplestore_puttupleslot` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `tuplestore_puttupleslot` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 8 步看 `下一轮
reset`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `下一轮 reset` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。
...
pg_backend_memory_contexts 采样 ExprContext。
...
断点 ExecMaterializeSlot。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

profile 观察 MemoryContextAlloc。
...
误区：看到 `per-output-tuple ExprContext`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `PlanState ps_ExprContext`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `virtual tuple result`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `materialized slot`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
CREATE TABLE t_reset AS SELECT g, repeat(md5(g::text), 5) v FROM generate_series(1,50000) g;
EXPLAIN (ANALYZE, VERBOSE) SELECT lower(v) || upper(v) FROM t_reset WHERE g < 1000;
```
...
压缩用户现象：围绕 `为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `为什么执行器要在每个 tuple 周期 reset per-tuple
context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `为什么执行器要在每个 tuple 周期 reset
per-tuple context，哪些结果必须复制或 materialize
才能跨周期保存？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `短命内存回收边界` 放回真实执行现场。主角是 per-tuple
context、virtual slot、materialize，主线是 一行 tuple 到下一行
tuple。
...
套回本节时，先写下 per-tuple context、virtual
slot、materialize 中哪个对象被创建、读取、失效或释放，再解释它如何影响 一行
tuple 到下一行 tuple。
...
在 `短命内存回收边界` 中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
