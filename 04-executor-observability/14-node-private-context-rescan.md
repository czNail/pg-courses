# PostgreSQL 节点私有 context 与 ExecReScan 状态重建边界

## 课程定位

前置知识：熟悉 ExecProcNode 拉取模型、work_mem、Sort / Hash /
Agg / Material 等阻塞或半阻塞节点。

本节唯一主问题：

```text
为什么执行节点不能只依赖 EState 的 per-query context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash table，并在 rescan 时选择重置、保留或重建？
```

核心矛盾：节点私有状态可能很大、可能需要跨多个 output tuple
保存，也可能能被重扫复用；但参数变化、方向变化、mark/restore 和 work_mem
又要求它们有清晰的失效边界。
...
本节只回答一个问题：为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？
...
```text
per-query context 管节点生命周期
per-tuple context 管每行临时结果
节点私有 context 管跨 tuple 算法状态
ExecReScan 决定复用、rewind、reset 或重建
```
...
| 阅读顺序 | 文件 | 本节关注点 |
| --- | --- | --- |
| 1 | src/backend/executor/execAmi.c | `ExecReScan()` 分派节点重新执行。 |
| 2 | src/backend/executor/nodeSort.c | `ExecInitSort()`、`ExecSort()`、`ExecReScanSort()`、`ExecEndSort()`。 |
| 3 | src/backend/executor/nodeMaterial.c | `ExecMaterial()`、`ExecReScanMaterial()`、`ExecEndMaterial()`。 |
| 4 | src/backend/executor/nodeHash.c | `ExecHashTableCreate()`、`ExecHashTableReset()`、`ExecHashTableDestroy()`。 |
| 5 | src/backend/executor/nodeHashjoin.c | `ExecHashJoinImpl()`、`ExecReScanHashJoin()`、`ExecEndHashJoin()`。 |
| 6 | src/backend/executor/nodeAgg.c | `hash_create_memory()`、`hash_agg_check_limits()`、`hash_agg_update_metrics()`。 |
| 7 | src/backend/utils/sort/tuplesort.c | `tuplesort_begin_*()`、`tuplesort_performsort()`、`tuplesort_end()`。 |
...
| 状态 | 本节语义 |
| --- | --- |
| tuplesortstate | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| tuplestorestate | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| HashJoinTable | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| HashAgg contexts | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| chgParam | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |
| eflags | 和 owner、生命周期、失效点一起解释，不能孤立读取。 |

`tuplesortstate`
的语义来自它被谁创建、谁更新、谁读取、何时失效。只看到字段值，还不能判断它是否仍然代表当前
tuple 或当前执行阶段。

诊断 `tuplesortstate` 时要同时记录所在 memory context 或上层
...
诊断 `tuplestorestate` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `HashJoinTable` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `HashAgg contexts` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `chgParam` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
诊断 `eflags` 时要同时记录所在 memory context 或上层
owner。executor 中大量 bug 和误判都来自把裸指针当成长期语义。
...
ExecSort  -> 改变或消费本节主状态
ExecMaterial  -> 改变或消费本节主状态
ExecHashJoinImpl  -> 改变或消费本节主状态
hash_agg_check_limits  -> 改变或消费本节主状态
ExecReScan  -> 改变或消费本节主状态
ExecEndNode  -> 改变或消费本节主状态
...
第 2 步看
`ExecSort`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecSort` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。
...
第 3 步看
`ExecMaterial`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecMaterial` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecMaterial` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 4 步看
`ExecHashJoinImpl`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecHashJoinImpl` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `ExecHashJoinImpl` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。

第 5 步看
`hash_agg_check_limits`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `hash_agg_check_limits` 前要问调用者能依赖什么：一个有效
slot、一个参数值、一个 context、一个 scan
descriptor、一个已格式化指标，还是一个需要 recheck 的候选结果。

如果 `hash_agg_check_limits` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
第 7 步看
`ExecEndNode`：先确认函数入口前状态是否已经初始化，再确认函数内部是否绑定、清理、重置或转交状态。

离开 `ExecEndNode` 前要问调用者能依赖什么：一个有效 slot、一个参数值、一个
context、一个 scan descriptor、一个已格式化指标，还是一个需要 recheck
的候选结果。

如果 `ExecEndNode` 是 hot
path，调试时用条件断点和计数，不要盲目逐步执行。executor 的调用栈很深，关键是状态边界。
...
EXPLAIN 观察 Sort Method 和 Hash Batches。

这个观测点只说明一个切面。要形成结论，必须把它和
owner、lifetime、reset/rescan 或 AM callback 对齐。

pg_backend_memory_contexts 观察 HashAgg context。
...
误区：看到 `tuplesortstate`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `tuplestorestate`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `HashJoinTable`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。

误区：看到 `HashAgg contexts`
就以为已经理解本节。实际还要知道它的创建者、读取者、失效条件和 cleanup 边界。
...
```text
SET work_mem = '1MB';
EXPLAIN (ANALYZE, VERBOSE, BUFFERS) SELECT * FROM generate_series(1,200000) g ORDER BY md5(g::text);
```
...
练习 1：打开
`/home/highgo/postgres/src/backend/executor/execAmi.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/execAmi.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 2：打开
`/home/highgo/postgres/src/backend/executor/nodeSort.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/nodeSort.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

练习 3：打开
`/home/highgo/postgres/src/backend/executor/nodeMaterial.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/nodeMaterial.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
练习 5：打开
`/home/highgo/postgres/src/backend/executor/nodeHashjoin.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/executor/nodeHashjoin.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。
...
练习 7：打开
`/home/highgo/postgres/src/backend/utils/sort/tuplesort.c`，找到本节主状态第一次被创建或绑定的位置。

继续在 `src/backend/utils/sort/tuplesort.c` 中找一个
cleanup、reset、recheck 或 instrumentation
边界，说明它如何保护本节唯一主问题。

### 14.1 从现象回到源码的排查剧本

压缩用户现象：围绕 `为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

选择断点入口：围绕 `为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录 owner：围绕 `为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录失效边界：围绕 `为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

记录返回语义：围绕 `为什么执行节点不能只依赖 EState 的 per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。

如果记录内容不能说明状态在何处变化，它就是背景材料，不应成为本节结论。

回到 EXPLAIN 或视图验证：围绕 `为什么执行节点不能只依赖 EState 的
per-query
context，而要拥有自己的内存上下文、tuplestore、tuplesort 或 hash
table，并在 rescan 时选择重置、保留或重建？`，只记录能解释本节主状态的事实。
...
复盘目标：把 `节点私有状态边界` 放回真实执行现场。主角是
tuplesort、tuplestore、hash table、rescan，主线是
首次执行到重扫复用。
...
套回本节时，先写下 tuplesort、tuplestore、hash table、rescan
中哪个对象被创建、读取、失效或释放，再解释它如何影响 首次执行到重扫复用。
...
在 `节点私有状态边界` 中，这条审查点必须能落到一个真实函数或结构体字段；否则它只是泛泛建议。
