...
本目录当前 55 节，已生成 12 个优化器主题组；后续建议追加 parser/analyzer/rewrite、ProcessUtility/DDL 和 planner 侧 partitioning 课程，用来补齐 planner 之前和 DDL 侧入口。

课程安排：

0. Parser / Analyzer / Rewrite：SQL 如何先变成语义化 `Query`。（待补）
...
13. ProcessUtility / DDL / event trigger。（待补）
14. Partitioning planner side。（待补）
...
以下课程只是计划占位，不代表文件已经生成。生成时继续使用追加编号，不重命名已有 01-55。

第 0 项 `Parser / Analyzer / Rewrite` 建议在阅读第 1 项 planner 入口之前先读；如果已经从 planner 开始，也应回补这组以理解 `Query` 从何而来：

1. 待补 `56-raw-parser-grammar-node-tags.md`：raw parser 如何从 SQL 文本生成 raw parse tree，gram.y、Node tag 和 location 如何服务后续错误定位？
2. 待补 `57-parse-analysis-name-resolution.md`：parse analysis 如何解析 range table、列名、namespace、scope 和 `ParseState`？
3. 待补 `58-type-coercion-operator-function-lookup.md`：类型推断、隐式 coercion、operator/function lookup 如何把语法表达式变成可执行语义？
4. 待补 `59-query-rewrite-view-rule.md`：view/rule rewrite 如何把用户 Query 改写成等价 Query，哪些边界会影响权限和可更新视图？
5. 待补 `60-rls-security-barrier-rewrite.md`：RLS、security barrier view 和 leakproof function 如何约束 qual 下推和 planner 搜索空间？

第 13 项 `ProcessUtility / DDL / event trigger` 建议作为 planner hook / utility hook 边界的补课，放在 planner 主流程之后阅读：

1. 待补 `61-processutility-ddl-dispatch.md`：`ProcessUtility()` 如何区分 DDL、transaction command、VACUUM、COPY、EXPLAIN 和其它 utility statement？
2. 待补 `62-ddl-command-catalog-update.md`：CREATE/ALTER/DROP 如何修改 catalog、记录 dependency、发出 invalidation，并在事务 abort 时回滚？
3. 待补 `63-event-trigger-processutility-boundary.md`：event trigger、ProcessUtility hook 和 executor/planner hook 的边界在哪里？
4. 待补 `64-ddl-locking-invalidation-order.md`：DDL 为什么必须先拿对象锁，再改 catalog，再发 invalidation，错误顺序会破坏什么？

第 14 项 `Partitioning planner side` 建议插在第 8 项 Base relation path 和第 9 项 Join search 之间阅读：

1. 待补 `65-partition-bound-catalog-model.md`：分区表的 bound、partition key、partition descriptor 和 relcache 如何表达分区空间？
2. 待补 `66-partition-pruning-planner.md`：planner-time partition pruning 如何根据约束、参数和表达式裁剪 Append 子路径？
3. 待补 `67-partitionwise-join-aggregate.md`：partition-wise join / aggregate 何时合法，为什么需要对齐 partition scheme 和 equivalence class？
4. 待补 `68-inheritance-expansion-appendrel.md`：inheritance/partition expansion 如何构造 appendrel、child rel 和 Var translation？
