...
本目录当前 46 节，包含 7 个主题组；Autovacuum / maintenance 调度课程已经把 VACUUM 语义连接到后台执行系统。
...
7. Autovacuum / maintenance / analyze 调度。
...
以下课程保留在 README 中作为建议阅读路径；编号沿用追加编号，未重命名已有 01-38。

第 7 项 `Autovacuum / maintenance / analyze 调度` 建议插在第 5 项 VACUUM / freeze / visibility map 之后阅读，再进入 Serializable：

1. [autovacuum launcher / worker 调度](39-autovacuum-launcher-worker-scheduling.md)：autovacuum launcher 如何扫描 database、调度 worker，并在全局 worker 限制下选择下一个维护目标？
2. [autovacuum threshold / scale factor](40-autovacuum-threshold-scale-factor.md)：insert/update/delete 计数、threshold、scale factor 和 reloptions 如何决定一张表是否触发 vacuum/analyze？
3. [autovacuum freeze / wraparound priority](41-autovacuum-freeze-wraparound-priority.md)：freeze age、multixact age 和 wraparound danger 如何让 anti-wraparound vacuum 压过普通成本策略？
4. [vacuum cost delay / throttling](42-vacuum-cost-delay-throttling.md)：vacuum cost accounting、delay 和 worker 平滑策略如何在维护吞吐与前台查询之间折中？
5. [parallel vacuum / index cleanup](43-parallel-vacuum-index-cleanup.md)：parallel vacuum 如何拆分 index cleanup，哪些状态仍必须由 leader 汇总？
6. [autovacuum ANALYZE 与 planner 统计信息刷新](44-analyze-autovacuum-statistics-refresh.md)：autovacuum analyze 如何刷新 planner 统计信息，为什么统计 stale 会跨到 optimizer 目录的问题域？
7. [maintenance diagnostics / age / bloat](45-maintenance-diagnostics-age-bloat.md)：如何把 `pg_stat_all_tables`、age、dead tuple、VM、bloat 和 logs 组合成维护诊断路径？
8. [autovacuum worker error / retry cleanup](46-autovacuum-worker-error-retry.md)：worker ERROR、锁冲突、表被删除和重试节流分别如何收尾？
