...
本目录当前 64 节，包含 10 个基础主题组；进程生命周期、重量级锁和 catalog/cache 基础设施课程已经补齐。
...
8. Postmaster / backend lifecycle / signal / background worker。
9. Heavyweight Lock Manager / Deadlock。
10. CatCache / RelCache / Shared Invalidation / Dependency。
...
以下课程保留在 README 中作为建议阅读路径；编号沿用追加编号，未重命名已有 01-42。

第 8 项 `Postmaster / backend lifecycle / signal / background worker` 建议插在第 1 项 Shmem 之后、第 4 项 PGPROC 之前阅读：

1. [postmaster bootstrap supervision](43-postmaster-bootstrap-supervision.md)：postmaster 如何完成集群级启动、监听、子进程监督和 crash policy？
2. [backend fork/exec bootstrap](44-backend-fork-exec-bootstrap.md)：客户端连接如何从 postmaster fork/exec 进入 backend bootstrap，哪些状态来自 inherited process image，哪些必须重新 attach？
3. [auth database connection startup：认证、database attach 与 startup GUC](45-auth-database-connection-startup.md)：认证、database attach、role/session 初始化和 GUC startup packet 如何形成 backend-local 初始状态？
4. [signal config reload interrupts](46-signal-config-reload-interrupts.md)：SIGHUP、SIGTERM、SIGINT、ProcDiePending、QueryCancelPending 和 CHECK_FOR_INTERRUPTS 如何在安全边界处理异步事件？
5. [background worker 注册生命周期](47-bgworker-registration-lifecycle.md)：background worker 如何注册、启动、重启、连接数据库并接入 latch / signal / shared memory？
6. [process exit crash cleanup](48-process-exit-crash-cleanup.md)：普通退出、FATAL、postmaster death 和 crash restart 分别由谁清理 backend-local 与 shared state？

第 9 项 `Heavyweight Lock Manager / Deadlock` 建议插在第 5 项同步原语之后、第 6 项 DSM 之前阅读：

1. [LOCKTAG 与 lock method table](49-locktag-lockmethod-table.md)：`LOCKTAG`、lock method table 和 lock mode 如何把 relation、page、tuple、transactionid、advisory lock 映射到统一锁管理器？
2. [heavyweight lock acquire/release](50-heavyweight-lock-acquire-release.md)：`LockAcquireExtended()` / `LockRelease()` 如何在 fast path、main lock table、resource owner 和 wait queue 之间推进？
3. [relation fast path lock 与 main lock table 边界](51-fastpath-lock-relation-boundary.md)：relation lock fast path 如何降低轻量访问成本，什么时候必须转入 main lock table？
4. [lock wait queue 与 deadlock detector](52-lock-wait-queue-deadlock-detector.md)：等待队列、soft edge、deadlock timeout 和 detector 如何判断是否必须 ERROR？
5. [transactionid lock wait](53-transactionid-lock-wait.md)：tuple update 冲突为什么常表现为等待 transactionid lock，而不是等待 heap tuple 本身？
6. [advisory lock session / transaction boundary](54-advisory-lock-session-xact.md)：session-level 与 transaction-level advisory lock 的 owner、释放边界和诊断方式有什么不同？
7. [lock diagnostics 与 blocking chain](55-lock-diagnostics-blocking-chain.md)：如何从 `pg_locks`、`pg_stat_activity` 和 wait event 回到 lock manager 源码等待点？

第 10 项 `CatCache / RelCache / Shared Invalidation / Dependency` 建议插在 ResourceOwner、PGPROC 和锁管理器之后，再进入 planner / executor 之前阅读：

1. [syscache / catcache lookup lifetime](56-syscache-catcache-lookup-lifetime.md)：syscache lookup 如何从 catalog tuple 构造 cache entry，refcount 与 memory context 如何避免悬挂 tuple？
2. [relcache build invalidation](57-relcache-build-invalidation.md)：relcache 如何从 `pg_class`、`pg_attribute`、index、rule、partition 信息构造 `Relation`，shared invalidation 如何让旧描述失效？
3. [typcache / operator cache / opclass cache](58-typcache-opclass-cache.md)：typcache、operator cache 和 opclass/opfamily cache 如何支撑表达式、排序、索引和 planner 判断？
4. [shared invalidation message flow](59-shared-invalidation-message-flow.md)：catalog 更新如何产生 invalidation message，backend 如何在 command boundary 消费并清理本地 cache？
5. [catalog snapshot / syscache consistency](60-catalog-snapshot-syscache-consistency.md)：catalog snapshot、syscache 命中和 invalidation 顺序如何共同保证 DDL/DML 看到一致的元数据？
6. [dependency recording / DROP CASCADE](61-dependency-recording-drop-cascade.md)：dependency 记录如何支撑 DROP RESTRICT/CASCADE、extension ownership 和对象生命周期？
7. [relmapper / relfilenode cache](62-relmapper-relfilenode-cache.md)：relfilenode、relmapper 和 mapped relation 如何连接 catalog identity 与物理文件 identity？
8. [catcache / relcache error cleanup](63-catcache-relcache-error-cleanup.md)：ERROR、transaction abort 和 invalidation 风暴下 cache ref、relcache entry 和 ResourceOwner 如何收尾？
9. [cache diagnostics / source entry map](64-cache-diagnostics-source-entry.md)：如何从 relcache/syscache 错误、stale metadata 和 invalidation 日志回到源码入口？
