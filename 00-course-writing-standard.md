...
新增课程如果在学习顺序上应插入已有课程之间，仍然采用追加编号，不重命名旧文件。

目录 README 负责维护推荐阅读顺序：

- 明确哪些课程已经生成。
- 明确哪些课程只是待补计划。
- 待补课程可以写计划文件名，但必须标注“待补”。
- 如果待补课程应插入已有课程之间，在 README 中写出推荐插入位置。
...
如果你要继续追 SQL 进入优化器前的路径，建议阅读待补课程
`56-parser-raw-query-boundary.md` 到 `60-rls-security-barrier-rewrite.md`，
再回到本目录第 1 节 planner 入口。
```

导航段只做跳转，不重复正文，不替代课程内容。
