...
前置知识：已理解 WAL insertion、record end LSN、page LSN、WAL-before-data，以及本地 WAL flush 和同步复制等待是不同层次。

本节唯一主问题：

```text
COMMIT 返回客户端之前，PostgreSQL 到底要等待本地 WAL、事务状态和同步备库推进到哪个边界？
```

核心矛盾：提交路径要尽快返回并复用 group commit；但一旦向客户端承诺 durability，就必须能在 crash recovery 和同步复制中解释这个承诺。

一句话运行模型：

```text
CommitTransaction() 调用 RecordTransactionCommit() 写 commit record；根据 synchronous_commit 选择 XLogFlush() 或 async commit；本地事务状态标记 committed 后，如需同步复制，再用 SyncRepWaitForLSN() 等待同步备库到配置要求的 write/flush/apply 边界。
```

学完后应能判断：commit record 插入和持久化的区别；`synchronous_commit=off/local/on/remote_write/remote_apply` 等待边界；group commit 为什么自然发生在 `XLogFlush()` 中；walwriter 为什么不能替代同步 commit 的本地 flush。

源码基线：本课基于本地 `/home/highgo/postgres` 源码，分支 `master`，提交 `bd4bd30ce6a7f08e95390c3fa068f2bfbe9fcee8`。

## 1. 本节在总主线中的位置

前面课程已经讲过 WAL record 如何进入 WAL buffers，以及数据页写出为什么要看 page LSN。本节转到事务提交：客户端看到 COMMIT 成功，究竟对应本地 WAL、`pg_xact` 和同步备库的哪几个边界。

它不展开 heap insert、checkpoint 或 WAL segment 回收，只沿着普通事务提交路径把 durability 承诺拆清楚。

## 2. 核心矛盾与一句话运行模型

提交路径既要快，又不能把“已返回客户端”建立在模糊状态上。PostgreSQL 通过本地 WAL flush、事务状态更新和可选同步复制等待三层边界，把性能选项和 durability 语义分开。
...
| 阅读顺序 | 文件 | 本节读什么 |
| --- | --- | --- |
| 1 | `src/backend/access/transam/xact.c` | `CommitTransaction()`、`RecordTransactionCommit()`、`XactLogCommitRecord()`、本地事务状态更新。 |
| 2 | `src/backend/access/transam/xlog.c` | `XLogFlush()`、group commit、`XLogWrite()`、WAL write/flush 边界。 |
| 3 | `src/backend/access/transam/xlogwait.c` | WAL wait/wakeup 和 flush 等待入口。 |
| 4 | `src/backend/replication/syncrep.c` | `SyncRepWaitForLSN()`、同步复制队列和等待级别。 |
| 5 | `src/backend/postmaster/walwriter.c` | walwriter 如何推进后台 WAL flush 与 async commit LSN。 |
| 6 | `src/backend/utils/misc/guc_tables.c`、`src/backend/utils/misc/guc_parameters.dat` | `synchronous_commit` 枚举与 GUC 入口。 |
| 7 | `src/backend/replication/walsender.c`、`src/include/replication/syncrep.h` | 备库反馈和同步复制状态边界。 |
...
## 6. 从 CommitTransaction 进入 RecordTransactionCommit
...
## 7. RecordTransactionCommit 的输入材料
...
## 10. XactLogCommitRecord 组装 commit record
...
## 13. RecordTransactionCommit 的 durability 分支
...
## 18. WAL insertion lock 的 wait/wakeup
...
## 20. commit_delay 与 commit_siblings
...
## 40. 崩溃边界：synchronous_commit=off
...
## 45. group commit 不是 batch commit
...
## 52. 实验二：比较 synchronous_commit=off
...
## 55. 常见误区一：COMMIT record 和 pg_xact 谁说了算
...
## 58. 常见误区四：remote_apply 更安全所以总该开
