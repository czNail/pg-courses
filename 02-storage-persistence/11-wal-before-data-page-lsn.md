...
前置知识：已理解 shared buffer、buffer descriptor、pin、content lock、`BM_DIRTY`、WAL record、record end LSN、checkpoint / bgwriter / backend eviction 都可能写出 dirty buffer。
...
核心矛盾：数据页写出必须尽量异步、合并、由后台推进；但 crash recovery 又要求磁盘上的每个数据页状态都能用已经持久化的 WAL 解释。
...
学完后应能判断：`PageSetLSN()` 应该由谁调用；`MarkBufferDirty()` 与 page LSN 为什么不是同一件事；`FlushBuffer()` 为什么是最后防线；permanent、unlogged、temporary relation 为什么不共享完全相同的持久化协议。

源码基线：本课基于本地 `/home/highgo/postgres` 源码，分支 `master`，提交 `bd4bd30ce6a7f08e95390c3fa068f2bfbe9fcee8`。

## 1. 本节在总主线中的位置

前面课程已经分别讲过 dirty flag、WAL record 和 record insertion。本节把它们合成一条 crash-safety 顺序：数据页可以晚写，但不能早于描述它的 WAL 持久化。

这个顺序连接 buffer manager、access method、WAL subsystem、checkpointer、bgwriter 和 backend eviction。后续 commit flush、segment 管理、FPI 和 redo contract 都会继续复用同一个 page LSN 边界。

## 2. 核心矛盾与一句话运行模型

本节的关键不是“每次修改都同步刷 WAL”。恰好相反，PostgreSQL 把 WAL insert、WAL flush 和 data page write 解耦，以获得吞吐；只有数据页真的要落盘时，`FlushBuffer()` 才读取 page LSN 并补齐 WAL flush。
...
这个不变量由调用方设置 page LSN，加上 `FlushBuffer()` 写出前 `XLogFlush(page_lsn)` 共同保证。

## 3. 核心文件分工与阅读顺序

| 阅读顺序 | 文件 | 本节读什么 |
| --- | --- | --- |
| 1 | `src/include/storage/bufpage.h` | `pd_lsn`、`PageGetLSN()`、`PageSetLSN()`。 |
| 2 | `src/backend/access/transam/xloginsert.c` | `XLogInsert()` / `XLogInsertRecord()` 返回 record end pointer 的语义。 |
| 3 | `src/backend/access/heap/heapam.c` | heap 修改 page、写 WAL、设置 page LSN 的代表路径。 |
| 4 | `src/backend/access/nbtree/nbtinsert.c` | btree insert/split 的 page LSN 设置边界。 |
| 5 | `src/backend/storage/buffer/bufmgr.c` | `MarkBufferDirty()`、`BufferGetLSN()`、`FlushBuffer()` 如何消费 page LSN。 |
| 6 | `src/backend/access/transam/xlog.c` | `XLogFlush()` 与 WAL durable boundary。 |
| 7 | `src/include/utils/rel.h`、`src/include/catalog/pg_class.h`、`src/backend/storage/buffer/localbuf.c` | permanent / unlogged / temp 边界。 |
| 8 | `src/backend/access/transam/README` | WAL-before-data 的设计说明。 |

## 4. 关键结论：WAL-before-data 顺序
...
## 6. `bufpage.h`：page LSN 是 page header 的字段
...
## 7. `XLogInsert()` 返回的是 page 应该记录的 LSN
...
## 9. Heap update / delete：多 page 时同一个 LSN 可覆盖多个 page
...
## 10. B-tree insert / split：为什么 fake LSN 也会出现
...
## 11. `MarkBufferDirty()` 与 page LSN 的职责分离
...
## 12. 为什么要先 `MarkBufferDirty()` 再 `XLogInsert()`
...
## 13. `FlushBuffer()`：WAL-before-data 的最后防线
...
## 14. `XLogFlush()`：请求 flush 到指定 record 位置
...
## 19. permanent / unlogged / temp：三个边界
...
## 20. `BM_PERMANENT`：FlushBuffer 判断的是 buffer 状态
...
## 21. unlogged relation：不是 crash-safe data page
...
## 22. temporary relation：local buffers，不进 shared WAL-before-data gate
...
## 23. crash safety 反例一：数据页早于 WAL 落盘
...
## 24. crash safety 反例二：page LSN 没有设置
...
## 25. crash safety 反例三：dirty flag 没有设置
...
## 26. crash safety 反例四：用 fake LSN 去 flush WAL
...
## 29. 源码跟读练习二：从 `XLogInsert()` 到 heap page
...
## 30. 源码跟读练习三：从 btree split 看多 page LSN
...
## 31. 源码跟读练习四：从写出路径收敛到 `FlushBuffer()`
...
## 32. 实验一：用 gdb 观察 heap insert 设置 page LSN

准备：
使用 `/home/highgo/postgres` 编译出的本地 PostgreSQL。
初始化一个测试实例。
建议打开 debug symbols。
步骤：
1. 启动 postgres。
...
## 33. 实验二：观察 `FlushBuffer()` 先刷 WAL 再写 data page
...
## 34. 实验三：比较 permanent、unlogged、temp
