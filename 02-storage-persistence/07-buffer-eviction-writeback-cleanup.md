...
前置知识：你已经读过本模块前六节，知道 `BufferTag` 如何标识 shared buffer，mapping table 如何 lookup，clock sweep 如何挑 candidate，也知道 dirty buffer、page LSN 和 WAL-before-data 的基本关系。
...
核心矛盾：buffer manager 希望 miss 尽快复用一个 slot；但 victim 可能 dirty，dirty page 可能需要先 flush WAL，数据页写出后还要登记 checkpoint fsync，请求 writeback 只是性能 hint，而且旧 tag 只有在没有并发 pin / dirty race 后才能从 mapping table 移除。
...
学完后应能判断：dirty victim 为什么不是 `StrategyGetBuffer()` 的职责；`FlushBuffer()` 为什么先 `XLogFlush(page LSN)`；writeback context 为什么不是 fsync；cleanup lock 与 replacement pin 为什么不是一回事。

源码基线：本课基于本地 `/home/highgo/postgres` 源码，分支 `master`，提交 `bd4bd30ce6a7f08e95390c3fa068f2bfbe9fcee8`。

## 1. 本节在总主线中的位置

上一节讲到 dirty page 的 page LSN。本节从 buffer miss 的后半段开始：`StrategyGetBuffer()` 已经选出并 pin 住一个 candidate，接下来要判断旧页能否真正离开这个 slot。

这条路径最容易被误读成一句话：clock sweep 淘汰一个页。实际上它至少拆成 candidate selection、dirty write、writeback hint、mapping invalidation 和 new tag installation。任何阶段都可能因为并发 pin、dirty race、content lock、WAL flush、I/O error 或 fsync queue fallback 改变成本和结果。

## 2. 核心矛盾与一句话运行模型

本节的模型有四个关键词：pin 防止 replacement slot 被别人抢走；content lock 防止写出时 page bytes 并发变化；I/O ownership 用 `BM_IO_IN_PROGRESS` 串行化同一 buffer 上的读写 I/O；durability 分层把 `smgrwrite()`、pending fsync 和 writeback hint 分开。

把这四层合在一起看，eviction 不是“移除缓存项”。它是一个跨 buffer manager、WAL、smgr、md、sync 和 OS page cache 的状态转移。
...
下面实验都可以在临时测试库执行。
不要在生产库运行。
路径假设你已经能从 `/home/highgo/postgres` 构建 PostgreSQL。
