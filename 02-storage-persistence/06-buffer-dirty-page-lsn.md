...
## 课程定位
本节主题：一个 shared buffer 被修改后，PostgreSQL 如何把它标记为 dirty，如何用 page LSN 把数据页写出和 WAL flush 绑定起来，以及 `MarkBufferDirty()` / `MarkBufferDirtyHint()` 在这个协议里到底承担什么职责。
本节唯一主问题：一个被修改的 page 如何从内存中的 dirty 状态，变成可以安全写回 relation 文件的状态？
核心矛盾：前台修改希望只留下 cheap dirty 标记并继续执行；但任何后续写出都必须证明 WAL 已经先于 data page 持久化。
本节主流程：访问方法修改 page -> `MarkBufferDirty()` -> `XLogInsert()` 返回 record end LSN -> `PageSetLSN()` -> `FlushBuffer()` 读取 page LSN -> `XLogFlush(page_lsn)` -> data page write。
## 源码基线
`/home/nail/postgres-lab`，`master` = `bd4bd30ce6a7f08e95390c3fa068f2bfbe9fcee8`。
## 阅读路径
...
- 典型调用点：`src/backend/access/heap/heapam.c`、`src/backend/access/nbtree/nbtinsert.c`、`src/backend/access/heap/heapam_visibility.c`
本节先给结论：
- `BM_DIRTY` 是 buffer descriptor 上的状态，含义是“buffer 中的 page 内容需要写回 relation 文件”。
- page LSN 是 page header 上的 `pd_lsn`，含义是“这个 page 当前内容依赖的最新 WAL record 的 end pointer”。
- WAL record LSN 在这里通常指 `XLogInsert()` / `XLogInsertRecord()` 返回的 record end pointer。
- 写脏页时，`FlushBuffer()` 会读取 page LSN，并在写 relation 文件前调用 `XLogFlush(page_lsn)`。
- 这就是 WAL-before-data：描述数据页修改的 WAL 必须先持久化，数据页才能被写出。
- `MarkBufferDirty()` 只负责让 buffer 进入 dirty 状态，不负责生成 WAL，也不负责设置 page LSN。
- 调用方必须按“修改 page、mark dirty、插入 WAL、设置 page LSN”的顺序完成协议。
- `MarkBufferDirtyHint()` 是 hint bit 等非关键修改的特殊入口，可能为了 checksum/torn page 风险额外写一条 FPI-for-hint WAL。
- 本基线没有 `BM_JUST_DIRTIED`；`buf_internals.h` 明确写着 flag bit 6 不再使用。
- `BM_JUST_DIRTIED` 是历史机制，曾用于处理“写出过程中又被 dirtied”的 race；本基线通过 share-exclusive content lock 让 flush 与 hint dirtier 互斥，因此不再需要。
---
## 1. 三个容易混淆的“脏”
阅读这节课时，先把三个层次分开：
1. buffer 是否 dirty。
...
---
## 6. `SyncOneBuffer()` 为什么依赖先 dirty 后 WAL
...
---
## 8. WAL record LSN：为什么是 end pointer
...
---
## 11. 典型调用点：btree insert 与 fake LSN
...
---
## 13. `StartSharedBufferIO()`：谁有权写这个 buffer
...
---
## 19. checkpoint 与 `BM_CHECKPOINT_NEEDED`
...
---
## 20. checkpoint 与 RedoRecPtr/FPI
...
但 crash 后丢失不会破坏逻辑数据。
---
## 23. hint bit 与 checksum/torn page 的冲突
...
---
## 24. `MarkSharedBufferDirtyHint()` 的顺序
...
---
## 25. WAL-before-data 的 crash 正确性
现在把协议串起来。
假设 heap page P 被修改。
修改方做：
...
---
## 27. permanent、unlogged、temp 的区别
...
---
## 32. 并发边界：header state 与 content lock 分工
...
---
## 33. 并发边界：checkpoint 漏掉后来的 dirty 为什么安全
...
如果不能，就不是 hint。
---
## 37. 跟读：从 dirty 到 writeback 的代码路径
建议按下面顺序跟读源码。
第一段：`buf_internals.h`。
...
---
## 39. 实验 1：静态 grep 跟踪标准顺序
在 `/home/nail/postgres-lab` 中执行：
```bash
...
重点是验证协议的普遍形状。
---
## 40. 实验 2：GDB 观察 insert 的 dirty/LSN 顺序
需要 debug build。
...
---
## 41. 实验 3：checkpoint 的 `BM_CHECKPOINT_NEEDED`
阅读 `BufferSync()`。
在 GDB 中断：
```gdb
break BufferSync
